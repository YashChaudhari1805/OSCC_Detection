...
def _check_file(file: UploadFile) -> None:
    ...
    if not file.filename or ext not in _ALLOWED:
        raise HTTPException(status_code=400, detail=f"Unsupported type. Allowed: {', '.join(_ALLOWED).upper()}.")
...
def auth_verify(request: Request, x_api_key: Optional[str] = Depends(_api_key_header)):
    rate_limit(request)
    if not settings.auth_enabled:
...
async def predict(file: UploadFile = File(...)):
    if not is_loaded():
        raise HTTPException(status_code=503, detail="Model not loaded. Run rebuild_model.py first.")
